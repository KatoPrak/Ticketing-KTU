...
<head>
    ...
    <style>
        ...
        .info-row { margin: 15px 0; padding: 12px; background: #f8f9fa; border-left: 4px solid #0d6efd; border-radius: 4px; }
        .info-row strong { color: #0d6efd; }
        ...
        @media only screen and (max-width: 620px) { body, .container { padding: 10px; } .btn { width: 100%; text-align: center; } }
    </style>
</head>
<body>
    <div class="container">
        ...
        <div class="content">
            ...
            <div class="info-row"><strong>ID Tiket:</strong> {{ $ticket->ticket_id }}</div>
            <div class="info-row"><strong>Kategori:</strong> {{ $ticket->category->name ?? '-' }}</div>
            <div class="info-row">
            ...
            <div class="info-row"><strong>Deskripsi:</strong><br>{!! nl2br(e($ticket->description)) !!}</div>
            <div class="info-row"><strong>Status:</strong> {{ strtoupper($ticket->status) }}</div>
            <div class="info-row"><strong>Waktu:</strong> {{ $ticket->created_at->format('d/m/Y H:i') }}</div>

            <a href="{{ url('/it/tickets') }}" class="btn">Lihat Detail Tiket</a>
        </div>
